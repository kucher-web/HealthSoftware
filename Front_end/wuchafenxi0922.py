...
def wuchafenxi_py(USER_POS,RV,ALPHA,W):
    ...
    Q2 = np.zeros((n, 4))
    Q3 = np.zeros((n, 4))
    Q1_STEP = np.zeros((n, 4))
    Q2_STEP = np.zeros((n, 4))
    for i in range(n)  :
    ...
    # 外推到整秒的四元数
    Q = np.array([OutExtra(Q2_STEP[i], W[i], -2.85) for i in range(n)])
    ...
    for i in range(n):
        ...
        omg = 7.2921151467e-5
        r = rv_temp[:3]
        v = rv_temp[3:] + omg * np.array([-r[1], r[0], 0])
        ...
        Porb = R_matrix @ (USER_POS - rv_temp[:3])
...
if __name__ == "__main__":
    ...
    RV = np.array([RV1, RV2, RV3, RV4, RV5, RV6])
    ...
    theta1,phi1,rang1 = wuchafenxi_py(USER_POS, RV, ALPHA , W)
    ...
    print("Phi1:", phi1)
    print("Range1:", rang1)
    print("Difference:", theta1 - theta)
    print("Difference1:", phi1 - phi)
    print("Difference2:", rang1 - rang)